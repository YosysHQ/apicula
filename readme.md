...
[^1]: `C` devices require passing the `--family` flag to Nextpnr as well as `--device` to `gowin_pack` because the C isn't part of the device ID but only present in the date code. Check `examples/Makefile` for the correct command.
